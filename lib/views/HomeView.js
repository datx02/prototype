module.exports = (function() {
  ...
  return Backbone.View.extend({
    ...
    showPublicKey: function() {
      if(this.session.generated_pubkey) {
        this.$el.find("#public-key .public-key").text(this.session.generated_pubkey);
      } else {
        App.rymdNode.getPublicKey(App.rymdNode.currentIdentity()).then(function(key){
          this.$el.find("#public-key .public-key").text(key);
        }.bind(this))
        .catch(function(err) {
          noty({
            type: 'error',
